...
validate_dotfiles_conf() {
    ...
    # Check for multiple DOTLOCAL definitions
    local dotlocal_count
    dotlocal_count=$(grep "^DOTLOCAL=" "$config_file" 2>/dev/null | wc -l | tr -d ' ')
    
    if [[ "$dotlocal_count" -gt 1 ]]; then
        error "Multiple DOTLOCAL definitions found in dotfiles.conf"
        error "Found $dotlocal_count DOTLOCAL definitions. Only one should be active."
        grep -n "^DOTLOCAL=" "$config_file" | while read -r line; do
            warning "  $line"
        done
    ...
    # Check if DOTLOCAL and symlink creation are both active
    if [[ "$dotlocal_count" -gt 0 ]] && grep -q "^ln -s.*local" "$config_file" 2>/dev/null; then
        warning "Both DOTLOCAL and symlink creation are active"
        echo -e "${YELLOW}         This may cause conflicts. Use only one method.${NC}" >&2
        warnings=1
    fi
    
    # Check if DOTLOCAL points to a non-existent directory
    if [[ "$dotlocal_count" -eq 1 ]]; then
        local configured_path
        configured_path=$(source "$config_file" 2>/dev/null && echo "${DOTLOCAL:-}" || true)
        if [[ -n "$configured_path" ]]; then
            # Expand tilde in path using common function
            local expanded_path=$(expand_path "$configured_path")
            if [[ ! -d "$expanded_path" ]]; then
                warning "DOTLOCAL points to non-existent directory:"
